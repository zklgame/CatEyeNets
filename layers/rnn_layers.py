...
def rnn_step_backward(dnext_h, cache):
    ...
    dnext_h_raw = (1 - next_h * next_h) * dnext_h
    dx = dnext_h_raw.dot(Wx.T)
    dprev_h = dnext_h_raw.dot(Wh.T)
    dWx = x.T.dot(dnext_h_raw)
    dWh = prev_h.T.dot(dnext_h_raw)
    db = np.sum(dnext_h_raw, 0)
...
def rnn_backward(dh, cache):
    ...
    """
    N, T, H = dh.shape
    x_0, prev_h_0, Wx_0, Wh_0, next_h_0 = cache[0]
    _, D = x_0.shape

    dx = np.zeros((N, T, D), dtype=x_0.dtype)
    dWx = np.zeros((D, H), dtype=Wx_0.dtype)
    dWh = np.zeros((H, H), dtype=Wh_0.dtype)
    db = np.zeros((H,), dtype=Wh_0.dtype)
